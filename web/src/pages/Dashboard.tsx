  ...
  return (
    <div className="card p-5">
      <div className="flex items-center justify-between mb-3">
        ...
        <Icon className="h-4 w-4 text-neutral-400 dark:text-neutral-500" aria-hidden="true" />
      </div>
...
const PIE_COLORS_LIGHT = ["#10b981", "#3b82f6", "#6366f1", "#94a3b8", "#ef4444"];
const PIE_COLORS_DARK = ["#34d399", "#60a5fa", "#818cf8", "#cbd5e1", "#f87171"];
...
export default function Dashboard() {
  ...
  if (error) {
    return <QueryError error={error as Error} onRetry={() => refetch()} message="Failed to load dashboard data." />;
  }
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-xs text-neutral-400 dark:text-neutral-500 mt-0.5">Platform overview</p>
        </div>
      ...
      {analytics && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
          <div className="card p-5">
            <span className="text-xs font-medium text-neutral-400 dark:text-neutral-500 uppercase tracking-wider">Success Rate</span>
            <p className="text-2xl font-bold text-neutral-900 dark:text-white mt-2">{analytics.tasks.success_rate}%</p>
            <p className="text-xs text-neutral-500 dark:text-neutral-400 mt-1">{analytics.tasks.completed}/{analytics.tasks.total} completed</p>
          </div>
          <div className="card p-5">
            <span className="text-xs font-medium text-neutral-400 dark:text-neutral-500 uppercase tracking-wider">Avg Completion</span>
            ...
            <p className="text-xs text-neutral-500 dark:text-neutral-400 mt-1">per task</p>
          </div>
          <div className="card p-5">
            <span className="text-xs font-medium text-neutral-400 dark:text-neutral-500 uppercase tracking-wider">Files Created</span>
            <p className="text-2xl font-bold text-neutral-900 dark:text-white mt-2">{analytics.files.total}</p>
            <p className="text-xs text-neutral-500 dark:text-neutral-400 mt-1">{(analytics.files.total_size_bytes / 1024 / 1024).toFixed(1)} MB</p>
          </div>
        </div>
      )}
          ...
          <div className="p-5">
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={DEMO_WEEKLY} barGap={2}>
                ...
                <XAxis dataKey="day" tick={{ fontSize: 11, fill: isDark ? "#9ca3af" : "#999" }} axisLine={false} tickLine={false} />
                <YAxis tick={{ fontSize: 11, fill: isDark ? "#9ca3af" : "#999" }} axisLine={false} tickLine={false} />
                <Tooltip
                  contentStyle={{
                    fontSize: 12,
                    borderRadius: 8,
                    border: isDark ? "1px solid #374151" : "1px solid #e5e5e5",
                ...
                <div className="flex flex-wrap gap-3 mt-2 justify-center">
                  {pieData.map((entry, i) => (
                    <div key={entry.name} className="flex items-center gap-1.5">
                      ...
                      <span className="text-[10px] text-neutral-500 dark:text-neutral-400 capitalize">{entry.name}</span>
                    </div>
                  ))}
                </div>
              </>
            ) : (
              <p className="text-sm text-neutral-400 dark:text-neutral-500 py-12">No data</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="card">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 divide-x divide-neutral-100 dark:divide-neutral-700">
          {["API Server", "PostgreSQL", "Redis", "MinIO Storage", "Mail Server", "WebSocket"].map((name) => (
            <div key={name} className="flex items-center gap-2.5 px-5 py-3.5">
              ...
              <div>
                ...
                <p className="text-[10px] text-emerald-600 dark:text-emerald-400">Healthy</p>
              </div>
            </div>
          ))}
        </div>
